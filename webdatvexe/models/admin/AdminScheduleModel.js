...
class AdminScheduleModel {
  ...
  async updateSchedule(id, scheduleData) {
    ...
    if (!route_id || !date || !departure_time || !arrival_time || !bus_type || !price || available_seats === undefined) {
    ...
    try {
      ...
      throw new Error('Lỗi khi cập nhật lịch trình');
    }
  }
  ...
  isValidDate(date) {
    ...
    // If date is a string in any reasonable format
  ...
  isValidTime(time) {
    ...
    // Handle time string in multiple formats
    const regex = /^([0-1]?[0-9]|2[0-3]):[0-5][0-9](:[0-5][0-9])?$/;
    
    // If it's already a valid time string
